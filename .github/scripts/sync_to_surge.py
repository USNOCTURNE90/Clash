...
def normalize(line: str):
    ...
    m = re.fullmatch(r'([^,/]+)(?:/(\d{1,2}))?(?:,(no-resolve))?', line, re.IGNORECASE)
    if m:
        raw_ip = m.group(1)
        mask = m.group(2)
...
def fix_current_repo_remote():
    ...
    run(['git', 'remote', 'set-url', 'origin', f'https://x-access-token:{token}@github.com/USNOCTURNE90/Clash.git'])
...
run(['git', 'clone', f"https://x-access-token:{os.environ['GITHUB_TOKEN']}@github.com/{os.environ['TARGET_REPO']}.git", 'surge_repo'])
...
for name in sorted(target_existing - source_names):
    already = any(x.get('repo') == 'Clash' and x.get('target_repo') == 'Surge' and x.get('filename') == name for x in pending)
    if not already:
        pending.append({'repo': 'Clash', 'target_repo': 'Surge', 'filename': name, 'requested_at': now_iso()})
        changed_local = True
...
if changed_remote:
    ...
    run(['git', '-C', 'surge_repo', 'remote', 'set-url', 'origin', f"https://x-access-token:{os.environ['GITHUB_TOKEN']}@github.com/{os.environ['TARGET_REPO']}.git"])
